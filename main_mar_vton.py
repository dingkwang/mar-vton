...
from util.loader import CachedFolder, CpDataset

from models.vae_vton import AutoencoderKL
from models import mar_vton
from engine_mar_vton import train_one_epoch, evaluate
import copy
...
def get_args_parser():
    ...
    parser.add_argument('--test_weight', default='', type=str, help='path to test weight')
...
def main(args):
    ...
    if args.use_cached:
        ...
        dataset_train = CpDataset(os.path.join(args.data_path, 'test'), transform=transform)
        dataset_test = CpDataset(os.path.join(args.data_path, 'test'), transform=transform)
    print(dataset_train)
    ...
    model = mar_vton.__dict__[args.model](
    ...
    if args.evaluate:
        ...
        evaluate(model_without_ddp, vae, ema_params, args, 0, batch_size=args.eval_bsz, log_writer=log_writer,
                 cfg=args.cfg, use_ema=True, data_loader_test=data_loader_test)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if test:
            ...
            evaluate(model_without_ddp, vae, ema_params, args, epoch, batch_size=args.eval_bsz, log_writer=log_writer,
                     cfg=1.0, use_ema=True, data_loader_test=data_loader_test)
